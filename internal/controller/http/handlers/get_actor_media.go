...
// All possible http responses for GetActorMedia handler
...
var (
	...
	ResponseGetActorMedia = Response{
		Code: http.StatusOK,
	}
)

// Input path parameter
const PathParamGetActorMediaID = "actor_id"

// Get all movies from database
func (h *Handlers) GetActorMedia(w http.ResponseWriter, r *http.Request) {
	...
	input := dto.GetActorMediaInput{}
	rp := NewRequestParams(log, r, &input)
	rp.AddPath(PathParamGetActorMediaID, &input.ActorID)
	...
	if err := rp.Parse(); err != nil {
		...
		RespondWithError(log, w, ErrGetActorMediaInvalidParams, err.Error())
		return
	}

	// Execute use case
	output, err := h.GetActorMediaUseCase.Execute(ctx, input)
	if err != nil {
		log.Error("Failed to fetch actor",
			log.ToString("error", err.Message),
		)
		// Respond with error, if use case execution fails
		RespondWithDTOError(log, w, ErrGetActorMediaInvalidParams, err)
		return
	}

	// Log successful completion
	log.Debug(
		"Fetching actor medias completed successfully",
