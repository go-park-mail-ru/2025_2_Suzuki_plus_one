...
func NewLoginUsecase(l logger.Logger, ur UserRepository, tr TokenRepository) *LoginUseCase {
	return &LoginUseCase{
		logger:    l,
		userRepo:  ur,
		tokenRepo: tr,
...
func (uc *LoginUseCase) Execute(ctx context.Context, input dto.PostAuthSignInInput) (dto.PostAuthSignInOutput, *dto.Error) {
	...
	if err := uc.tokenRepo.AddNewRefreshToken(ctx, user.ID, refreshToken, expiration); err != nil {
		derr := dto.NewError(
			"usecase/post_auth_signin",
			entity.ErrPostAuthSignInNewRefreshTokenFailed,
			err.Error(),
